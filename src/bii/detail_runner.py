...
def run_ready_detail_checkpoint(
    ...
    *,
    ...
    uploads, or publishes raw source material. The reviewed source-access policy and
    run summary are persisted before and after the run.
    """
    ...
    run_id = store.start_detail_collection_run(
        validation_label=validation_label,
        checkpoint_n=checkpoint_n,
        policy_review_id=policy_review_id,
        started_at=started_at,
        raw_directory=str(raw_directory),
        notes="Private staged DIFE detail collection.",
    )
    ...
    try:
        ...
        store.finish_detail_collection_run(
            run_id,
        ...
        store.finish_detail_collection_run(
            ...
            failed=failed,
            status="FAILED",
            notes=f"Run aborted by unexpected error: {type(exc).__name__}: {exc}",
        )
        raise
